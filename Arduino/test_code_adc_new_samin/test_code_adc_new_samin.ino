...
double sumIn = 0;                            //value of BISR sum voltage  
double sumOut = 0; 
double sum2 = 0;                            //value of value (sum/i) sum voltage  
...
double R2 = 0;                              //gain of the inputs (value sum converted)
double Voltage_ADC = 0;                     //initialize voltage value for ADC
double Gain_Const = 0.0001875;              //constant for defult ADC gain  0.0001875 to get voltage
...
double global = 35;                         //global variable for potentiometer
...
double BluetoothISR(double sumIn, sumOut int i2){
String inputString = "";
...
void loop(){
  ...
  for (i = 1; i <= (index); i++) {
    ...
    inputvalue = (abs(adc0*Gain_Const));                     //absolute analog from digital voltage value (V) 
    Serial.print("ADC input voltage value: ");              // print analog value
    Serial.println(inputvalue, 7);
    delay(10);

    adc1 = ads.readADC_SingleEnded(1);               //reads output voltage value
    outputvalue = (abs(adc1*Gain_Const));
    ...
    Serial.println(inSum, 7);
    Serial.print("Input AVG: ");
    Serial.println(avg, 7);
    inAvg = inSum/i; 
    ...
    Serial.println(outSum, 7);
    Serial.print("Output AVG: ");
    Serial.println(outAvg, 7);
    outAvg = outSum/i; 

    BISR = BluetoothISR(inSum, outSum, i);
    Serial.print("Initial BISR: ");
    Serial.println(BISR);
    

      if (((BISR >= 0.0001) && (avg > BISR2)) || ((BISR >= 0.0001) && (avg < BISR3))){
      Serial.println("______________________________________________________________________________");
      
      if (R1 == 0) { 
       Serial.print("BISR: ");
       Serial.println(BISR, 7);
        ...
        sum2 = sum/i;
        Serial.print("sum2: ");
        Serial.println(sum2, 7);   
        R2 = sum2*conv;                              //Current Sum
        Serial.print("R2: ");
        Serial.println(R2, 7);
        sum1 = R1/conv;
        Serial.print("sum1: ");
        Serial.println(sum1, 7);
        
        if ((sum1 < sum2) && ((abs((sum1 - (sum2 - sum1))*conv)) >= 12)) {
          R = abs(((sum1 - (sum2 - sum1))*conv));
          Serial.print("R: ");
          Serial.println(R);
          for (i4 = R1; i4 >= R; i4--){
        ...
        else if ((sum1 > sum2) && (((sum1 + (sum1 - sum2))*conv) <= 100)) {
          R = abs(((sum1 + (sum1 - sum2))*conv));
          Serial.print("R: ");
          Serial.println(R);
          for (i4 = R1; i4 <= R; i4++){
           digitalPotWrite(i4);                               //write the potentiometer to the set threshold
        ...
        if ((sum1 <= 10) && (((sum1 + (sum1 - sum2))*conv) <= 20)){
          R = abs(((sum1 + (sum1 - sum2))*conv));
          for (i4 = R1; i4 <= R; i4++){
           digitalPotWrite(i4);                               //write the potentiometer to the set threshold
           delay(10);
    ...
    totalavg = sum/index;                            //average the values of the array
    ...
    Serial.print("                 Total Average (wiper):                    ");
    Serial.println(avg*conv);
    delay(10);
}
  //main code end
