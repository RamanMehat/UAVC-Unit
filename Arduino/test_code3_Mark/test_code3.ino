...
//      global = BR;
//     }
//     BR1 = abs(((global - volchange)*(Vi/conv)));
//     Serial.print("BR1: ");
//     Serial.println(BR1);
//     return BR1;
//    }
...
// Display status of Sound Detector
   Serial.println("Initialized");
    ...
    digitalPotWrite(global);                      //initally set potentiometer on startup
}
 
void loop(){
  //main code start
  double BISR2 = BISR + (BISR*0.20);  
  double BISR3 = BISR - (BISR*0.20);
  double value;
  int i;
  double sum = 0;
  double RMSsum = 0;
  ...
  for (i = 1; i <= (index); i++) {
    ...
    R1 = (conv*BISR)/Vi;
    global2 = (RMSsum/i)*conv/Vi;
    
      if (BISR >= 1){
           
          if((RMSsum/i) > (BISR2){ 
            if((global2 - (global2 - (BISR2*(Vi/conv)))) > 10){  
              R1 = global2 - (global2 - (BISR2*(Vi/conv)));
              global = R1;
              digitalPotWrite(R1); 
              Serial.print("Wiper Value (decrease): ");              //Value of wiper outputted - set to initial threshold
              Serial.println(R1); 
            }
           }
        
          if((RMSsum/i) < (BISR3){
            if((global2 + (global2 - (BISR2*(Vi/conv)))) < 85){  
            R1 = global2 + ((BISR2*(Vi/conv)) - global2);
            global = R1;
            digitalPotWrite(R1); 
            Serial.print("Wiper Value (increase): ");              //Value of wiper outputted - set to initial threshold
            Serial.println(R1);  
            }
          }
     }
